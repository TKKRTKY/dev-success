...
作りたいものや技術スタック、実行エージェントなどをタブ形式で設定します。

![初期設定ページ](docs/screenshot-setup.png)

### メインページ

「考える」「作る」「確かめる」「整える」などのコマンドを選び、ターン制で開発を進めます。

![メインページ](docs/screenshot-main.png)

## Dev Successとは

Dev Successは、アプリ開発を「考える」「作る」「確かめる」「整える」のターンで進めるツールです。

人間が毎回ゼロからプロンプトを書くのではなく、「考える」で提示される3つのアイデアから次に育てる機能を選びます。選んだアイデアは`FeatureSeed`（機能の元）として保存され、「作る」でCodex / Claude Code / OpenCodeなどのAIエージェントへ実装を依頼できます。
...
- npm
- Git
- CLIエージェント連携を使う場合は、対応するCLI（Codex / Claude Code / OpenCode）
- 操作対象となるローカルGitリポジトリ
...
6. いずれかのエージェントで実行するか、プロンプトをコピーして手動実装する
...
- CLIエージェントはファイルを作成・変更・削除する可能性があります
